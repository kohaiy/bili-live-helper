...
export const playByTencentTTS = async (info: VoiceInfo) => {
    try {
        const { secretId, secretKey } =
            config.value.basic?.broadcaseVoiceTencentTTS ?? {};
        if (!secretId || !secretKey) {
            return;
        }
        const [err, data] = await IpcRendererUtil.send<[Error | null, string?]>(
            'TTS',
            {
                Text: info.text,
        ...
        console.error(e);
        throw new Error(`[腾讯云TTS] ${e?.message ?? '生成语音失败'}`);
    }
};
...
const getVoiceType = (uid: number) => {
    ...
    voiceTypeMap[uid] =
        voiceTypes[Math.floor(Math.random() * voiceTypes.length)].code;
    return voiceTypeMap[uid];
};

export const resetVoiceType = (uid: number, msg: string) => {
    if (/^(变声|bs)\s+l/.test(msg)) {
        return voiceTypes;
    }
    if (/^(变声|bs)\s+\d+/.test(msg)) {
        const code = msg.match(/^(变声|bs)\s+(\d+)/)?.[2];
        if (code && voiceTypes.find((item) => item.code === +code)) {
            voiceTypeMap[uid] = +code;
            return [];
        }
    }
    if (/^(变声|bs)\s+\S+/.test(msg)) {
        const keyword = msg.match(/^(变声|bs)\s+(\S+)/)?.[2];
        const code = keyword ? voiceTypes.find((item) => item.desc.includes(keyword))?.code : undefined;
        if (code) {
            voiceTypeMap[uid] = code;
            return [];
        }
    }
    delete voiceTypeMap[uid];
    return [];
};
