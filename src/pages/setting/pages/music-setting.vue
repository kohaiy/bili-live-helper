  ...
  <setting-container title="点歌设置" @confirm="handleSave">
    <div class="music-setting">
      <a-form :model="form" :rules="rules" ref="form$" label-position="top" label-suffix=" :">
        ...
        <a-form-item label="歌单上限" v-show="form.enable" field="listLimit">
...
const form = ref({
  ...config.value.music,
  defaultListId: String(config.value.music?.defaultListId ?? ''),
...
const handleSave = async () => {
  form$.value?.validate(async (errors) => {
    if (!errors) {
      config.value.music = {
        ...form.value,
        defaultListId: form.value.defaultListId ? +form.value.defaultListId : undefined
      };
