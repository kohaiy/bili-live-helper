...
import { handleAutoReply } from './handles';

const { addMessage, resetVoiceType } = useVoice();
...
onAll((body) => {
    ...
    if (/^(变声)|(bs)/.test(body.msg)) {
        const voiceTypes = resetVoiceType(body.uid, body.msg);
        if (voiceTypes.length) {
            sendNotify({
                title: '音色列表',
                markdown: true,
                content: '> 发送 ** bs + 空格 + 前面的数字 ** \n\n'
                    + voiceTypes.map(item => `- \`${item.code}\` - ${item.desc}`).join('\n'),
                duration: 20000,
            });
        }
    }
